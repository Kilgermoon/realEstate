					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$invoices = $conn->query("SELECT p.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name FROM water&gabbage_payments p inner join tenants t on t.id = p.tenant_id where t.status = 1 order by date(p.date_created) desc ");
...
<script>
	...
	$('#new_invoice').click(function(){
		uni_modal("New invoice","water_gabbageManage.php","mid-large")
		
	})
	$('.edit_invoice').click(function(){
		uni_modal("Manage invoice Details","water_gabbageManage.php?id="+$(this).attr('data-id'),"mid-large")
