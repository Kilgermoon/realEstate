...
Class Action {
	...
	function get_tdetails(){
		...
		$paid = $this->db->query("SELECT SUM(amount) as paid FROM water_gabbage_payments where id != '$pid' and tenant_id =".$id);
		$last_payment = $this->db->query("SELECT * FROM water_gabbage_payments where id != '$pid' and tenant_id =".$id." order by unix_timestamp(date_created) desc limit 1");
	...
	function save_payment(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO water_gabbage_payments set $data");
			$id=$this->db->insert_id;
		}else{
			$save = $this->db->query("UPDATE water_gabbage_payments set $data where id = $id");
		}
	...
	function delete_payment(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM water_gabbage_payments where id = ".$id);
		if($delete){
			return 1;
		}
	}
