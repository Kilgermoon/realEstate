...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['temp_user'])) {
        ...
        // Further processing with $temp_user
    } else {
        // Handle the case where 'temp_user' is not set in the session
        echo "Temp user session key is not set.";
    }
    if($qry->num_rows > 0){
        foreach ($qry->fetch_array() as $key => $value) {
            if($key != 'otp' && !is_numeric($key))
                $_SESSION['temp_user'.$key] = $value;
        }
        if($_SESSION['email'] < 0){
