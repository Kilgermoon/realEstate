						...
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								...
								while($row=$tenants->fetch_assoc()):
									...
									$paid = $conn->query("SELECT SUM(amount) as paid FROM water_gabbage_payments where tenant_id =".$row['id']);
									$last_payment = $conn->query("SELECT * FROM water_gabbage_payments where tenant_id =".$row['id']." order by unix_timestamp(date_created) desc limit 1");
...
<script>
	...
	$('#new_payment').click(function(){
		uni_modal("New payment","water_gabbegeManage.php","mid-large")
		
	})
	$('.edit_payment').click(function(){
		uni_modal("Manage payment Details","water_gabbegeManage.php?id="+$(this).attr('data-id'),"mid-large")
		
	})
	$('.view_payment').click(function(){
		uni_modal("Tenants Payments","water_gabbageView.php?id="+$(this).attr('data-id'),"mid-large")
	...
	function delete_payment($id){
		start_load()
		$.ajax({
			url:'ajax2.php?action=delete_payment',
			method:'POST',
			data:{id:$id},
